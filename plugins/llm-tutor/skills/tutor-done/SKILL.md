...
Award +0.5 cycles either way. Reward is symmetric so users can't game by always thumbs-upping. Cap-aware:
...
If substantive: award +1 cycle (cap-respecting):
...
2. **Thumbs reward is symmetric.** Always +0.5 cycles either way. If you give 👎 less reward than 👍, the metric becomes useless.
3. **Substantive guard for the +1 cycle.** Length > 20 chars AND not in the skip list. This is the anti-gaming defense.
4. **Cap-respect cycles rewards.** Never push cycles over `cycles_cap`. Use `jq … | min` for the capping math.
...
8. **Don't lecture about the cycles rewards.** Mention them once in passing ("+0.5 cycles", "+1 cycle"). Don't explain the gamification economy in the closing message.
...
- **Cycles already at cap** → reward computes to no-change; print "+0.5 cycles (capped at $CAP)". Be honest about the cap.
